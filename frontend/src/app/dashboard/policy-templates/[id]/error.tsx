...
export default function PolicyTemplateDetailError({ error, reset }: ErrorProps) {
  const router = useRouter();

  useEffect(() => {
    // Log the error to an error reporting service
    console.error('Policy Template detail page error:', error);
  }, [error]);
  ...
  return (
    <div className="space-y-6">
      ...
      <Breadcrumb items={[
        ...
        { label: "Policy Details", current: true }
      ]} />
      ...
      <Card className="max-w-2xl mx-auto">
        ...
        <CardContent className="text-center space-y-4">
          <p className="text-gray-600">
            We encountered an error while loading the policy template details. 
